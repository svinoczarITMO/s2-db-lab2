...
from "Н_ЛЮДИ"
    where "ОТЧЕСТВО" in (
        ...
        join "Н_ПЛАНЫ" П on Н_УЧЕНИКИ."ПЛАН_ИД" = П."ОТД_ИД"
        join "Н_ОТДЕЛЫ" О on П."ИД" = О."ИД"
        join "Н_УЧЕНИКИ" У on "Н_ЛЮДИ"."ИД" = У."ЧЛВК_ИД"
        where О."КОРОТКОЕ_ИМЯ" = 'КТиУ'
...
select
  uch.ГРУППА,
  count(case when ved.ОЦЕНКА = '5' then 1 end) as "Отличники",
  count(case when ved.ОЦЕНКА = '2' then 1 end) as "Двоечники"
from Н_УЧЕНИКИ uch
join Н_ВЕДОМОСТИ ved on (uch.ЧЛВК_ИД=ved.ЧЛВК_ИД)
where (ved.ОЦЕНКА in ('5','2'))
group by uch.ГРУППА;
...
where НВ."ОЦЕНКА" = '2' group by НЛ."ИД") as bufftable;

select count(*) from (select НЛ."ИД" from "Н_УЧЕНИКИ" НУ
inner join "Н_ПЛАНЫ" НП on (НУ."ПЛАН_ИД"=НП."ИД")
inner join "Н_ЛЮДИ" НЛ on НУ."ЧЛВК_ИД" = НЛ."ИД"
inner join "Н_ВЕДОМОСТИ" НВ on НЛ."ИД" = НВ."ЧЛВК_ИД"
where НВ."ОЦЕНКА" = '5' group by НЛ."ИД") as bufftable;